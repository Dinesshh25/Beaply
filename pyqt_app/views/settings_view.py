...
class SettingsView(QWidget):
    ...
    def _build(self):
        ...
        sub = QLabel("Manage your account preferences and security")
        ...
        pwll.setSpacing(0)
        pwll.addWidget(self._bold("Change Password", 12))
        pwll.addWidget(self._muted("Change password to keep account secure"))
        ...
        emll.setSpacing(0)
        emll.addWidget(self._bold("Email Verification", 12))
        emll.addWidget(self._muted(f"Verified email: {profil.get('email', 'N/A')}"))
        ...
        thll.addWidget(self._muted("Select application theme"))
        ...
        lnll.setSpacing(0)
        lnll.addWidget(self._bold("Language", 12))
        lnll.addWidget(self._muted("Select interface language"))
        ...
        tsll.setSpacing(0)
        tsll.addWidget(self._bold("Text Size", 12))
        tsll.addWidget(self._muted("Select text size"))
    ...
    def _set_size(self, val):
        ...
        QMessageBox.information(self, "Info", "Text size changes will apply after restart.")

    def _del_acct(self):
        r = QMessageBox.question(self, "Delete Account", "Are you sure? This can't be undone.")
        if r == QMessageBox.StandardButton.Yes:
            ok, msg = hapus_akun(self._pid, True)
            if ok:
                QMessageBox.information(self, "Done", "Account deleted.")
                if self._refresh_cb:
                    self._refresh_cb()
            else:
    ...
    def _change_pw(self):
        ...
        dl.addWidget(self._bold("Change Password", 15))
        e1 = QLineEdit()
        e1.setPlaceholderText("Current Password")
        e1.setEchoMode(QLineEdit.EchoMode.Password)
        e1.setFixedHeight(38)
        dl.addWidget(e1)
        e2 = QLineEdit()
        e2.setPlaceholderText("New Password")
        e2.setEchoMode(QLineEdit.EchoMode.Password)
        e2.setFixedHeight(38)
        dl.addWidget(e2)
        e3 = QLineEdit()
        e3.setPlaceholderText("Confirm New Password")
        ...
        def save():
            if not e1.text() or not e2.text() or not e3.text():
                ...
                return
            if e2.text() != e3.text():
                QMessageBox.warning(dlg, "Error", "Passwords don't match!")
                return
            QMessageBox.information(dlg, "Success", "Password changed!")
            dlg.accept()
        sb = QPushButton("Change Password")
