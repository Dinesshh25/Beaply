...
Beaply - View: Profil & Home

Dipindahkan dari: Profile_dan_Setting/gui_profile.py
...
from controllers.profil_controller import (
    ...
    tampil_semua_profil, simpan_data_opsional,
)
...
class HalamanHome(ctk.CTkFrame):
    ...
    def _pilih_profil(self):
        profils = tampil_semua_profil(self.user_id)
        if not profils:
...
class PilihProfilWindow(ctk.CTkToplevel):
    ...
    def _build(self):
        ...
        for i, p in enumerate(self.profils):
            ...
            ctk.CTkLabel(inner,
                         text=f"{p['jenjang']} — {p['jurusan']} • {p['kampus']}",
...
class HalamanBuatProfil(ctk.CTkFrame):
    ...
    def _form_wajib(self, parent):
        ...
        self.dd_jenjang.set("S1")
        self.dd_jenjang.pack(side="left", padx=8)
        ...
        self.dd_jk.set("Laki-laki")
        self.dd_jk.pack(side="left", padx=8)
    ...
    def _form_spesifik(self, parent):
        ...
        self.dd_jlpt.set("")
        self.dd_jlpt.pack(side="left", padx=8)
...
class HalamanProfil(ctk.CTkFrame):
    ...
    def _build(self):
        ...
        for lbl, key in [("IELTS","skor_ielts"), ("TOEFL iBT","skor_toefl"),
                         ("Duolingo","skor_duolingo"), ("SAT","skor_sat"),
                         ("ACT","skor_act"), ("GRE","skor_gre"),
                         ("GMAT","skor_gmat"), ("HSK","skor_hsk"),
                         ("JLPT","level_jlpt")]:
            val = profil.get(key)
            is_empty = val is None or val == 0 or str(val).strip() == ""
    ...
    def _show_optional_form(self):
        ...
        ctk.CTkLabel(inner, text="JLPT Level", width=180, anchor="w",
                     font=ctk.CTkFont(size=12, weight="bold"),
                     text_color=TEXT_DARK).pack(side="left")
        self._jlpt_dd = ctk.CTkComboBox(inner,
                                        values=["", "N1", "N2", "N3", "N4", "N5"],
                                        width=160, height=32)
        self._jlpt_dd.set(profil.get("level_jlpt", "") or "")
    ...
    def _save_optional(self):
        ...
        ok, msg = simpan_data_opsional(self.profil_id, updates)
        if ok:
